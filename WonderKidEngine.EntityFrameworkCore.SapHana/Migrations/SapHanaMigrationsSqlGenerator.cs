...
namespace WonderKidEngine.EntityFrameworkCore.SapHana.Migrations
{
    // CHECK: Can we increase the usage of the new model over the old one, or are we done here?
    /// <summary>
    ///     SapHana-specific implementation of <see cref="MigrationsSqlGenerator" />.
    /// </summary>
    public class SapHanaMigrationsSqlGenerator : MigrationsSqlGenerator
    {
        ...
        public SapHanaMigrationsSqlGenerator(
        ...
        protected override void Generate(MigrationOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            switch (operation)
            {
                case SapHanaCreateDatabaseOperation createDatabaseOperation:
                    Generate(createDatabaseOperation, model, builder);
                    break;
                case SapHanaDropDatabaseOperation dropDatabaseOperation:
                    Generate(dropDatabaseOperation, model, builder);
                    break;
                case SapHanaDropPrimaryKeyAndRecreateForeignKeysOperation dropPrimaryKeyAndRecreateForeignKeysOperation:
                    Generate(dropPrimaryKeyAndRecreateForeignKeysOperation, model, builder);
                    break;
                case SapHanaDropUniqueConstraintAndRecreateForeignKeysOperation dropUniqueConstraintAndRecreateForeignKeysOperation:
        ...
        protected virtual void CheckSchema(MigrationOperation operation)
        {
            ...
            if (schema != null)
            {
                ...
                throw new InvalidOperationException($"A schema \"{schema}\" has been set for an object of type \"{operation.GetType().Name}\"{(string.IsNullOrEmpty(name) ? string.Empty : $" with the name of \"{name}\"")}. SapHana does not support the EF Core concept of schemas. Any schema property of any \"MigrationOperation\" must be null. This behavior can be changed by setting the `SchemaBehavior` option in the `UseSapHana` call.");
            }
        }
            ...
            bool terminate = true)
        {
            ...
            if (operation[SapHanaAnnotationNames.CharSet] is string charSet)
            {
            ...
            {
                tableOptions.Add(("COMMENT", SapHanaStringTypeMapping.EscapeSqlLiteralWithLineBreaks(operation.Comment, !_options.NoBackslashEscapes, false)));
            }

            tableOptions.AddRange(
                SapHanaEntityTypeExtensions.DeserializeTableOptions(operation[SapHanaAnnotationNames.StoreOptions] as string)
        ...
        {
            var oldCharSet = operation.OldTable[SapHanaAnnotationNames.CharSet] as string;
            var newCharSet = operation[SapHanaAnnotationNames.CharSet] as string;
...
DEALLOCATE PREPARE __pomelo_SqlExprExecute;";
            ...
            var oldTableOptions = SapHanaEntityTypeExtensions.DeserializeTableOptions(operation.OldTable[SapHanaAnnotationNames.StoreOptions] as string);
            var newTableOptions = SapHanaEntityTypeExtensions.DeserializeTableOptions(operation[SapHanaAnnotationNames.StoreOptions] as string);
            ...
            //    {
                    ...
                    if (index == null)
                    {
                        throw new InvalidOperationException(
                            $"Could not find the model index: {Dependencies.SqlGenerationHelper.DelimitIdentifier(operation.Table, operation.Schema)}.{Dependencies.SqlGenerationHelper.DelimitIdentifier(operation.NewName)}. Upgrade to SapHana 5.7+ or split the 'RenameIndex' call into 'DropIndex' and 'CreateIndex'");
                    }
        ...
        /// /// <summary>
        ///     Ignored, since schemas are not supported by SapHana and are silently ignored to improve testing compatibility.
        /// </summary>
        /// <param name="operation"> The operation. </param>
        /// <param name="model"> The target model which may be <see langword="null"/> if the operations exist without a model. </param>
        /// <param name="builder"> The command builder to use to build the commands. </param>
        ...
        /// <summary>
        ///     Builds commands for the given <see cref="SapHanaCreateDatabaseOperation" />
        ///     by making calls on the given <see cref="MigrationCommandListBuilder" />.
        /// </summary>
        /// <param name="operation"> The operation. </param>
        /// <param name="model"> The target model which may be <see langword="null"/> if the operations exist without a model. </param>
        ...
        protected virtual void Generate(
            [NotNull] SapHanaCreateDatabaseOperation operation,
            [CanBeNull] IModel model,
            [NotNull] MigrationCommandListBuilder builder)
        {
            Check.NotNull(operation, nameof(operation));
        ...
        /// <summary>
        ///     Builds commands for the given <see cref="SapHanaDropDatabaseOperation" />
        ///     by making calls on the given <see cref="MigrationCommandListBuilder" />.
        /// </summary>
        /// <param name="operation"> The operation. </param>
        /// <param name="model"> The target model which may be <see langword="null"/> if the operations exist without a model. </param>
        /// <param name="builder"> The command builder to use to build the commands. </param>
        protected virtual void Generate(
            [NotNull] SapHanaDropDatabaseOperation operation,
            [CanBeNull] IModel model,
            [NotNull] MigrationCommandListBuilder builder)
        {
            Check.NotNull(operation, nameof(operation));
        ...
        protected override void Generate(AlterDatabaseOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            var oldCharSet = operation.OldDatabase[SapHanaAnnotationNames.CharSet] as string;
            var newCharSet = operation[SapHanaAnnotationNames.CharSet] as string;
        ...
        protected override void Generate(
            ...
            MigrationCommandListBuilder builder)
            => Generate(
                new SapHanaDropUniqueConstraintAndRecreateForeignKeysOperation
                {
        ...
        protected virtual void Generate(
            SapHanaDropUniqueConstraintAndRecreateForeignKeysOperation operation,
            IModel model,
            MigrationCommandListBuilder builder)
        {
            Check.NotNull(operation, nameof(operation));
            ...
            // alternate key exist, if explicitly requested by the user via `SapHanaMigrationBuilderExtensions.DropUniqueConstraint()`.
            ...
            [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            var valueGenerationStrategy = SapHanaValueGenerationStrategyCompatibility.GetValueGenerationStrategy(operation.GetAnnotations().OfType<IAnnotation>().ToArray());

            var autoIncrement = false;
            if (valueGenerationStrategy == SapHanaValueGenerationStrategy.IdentityColumn &&
            ...
            if (operation.IsRowVersion || valueGenerationStrategy == SapHanaValueGenerationStrategy.ComputedColumn)
            {
                switch (matchType)
                {
                    case "datetime":
        ...
        private void GenerateComment(string comment, MigrationCommandListBuilder builder)
        {
            ...
            builder.Append(" COMMENT ")
                .Append(SapHanaStringTypeMapping.EscapeSqlLiteralWithLineBreaks(comment, !_options.NoBackslashEscapes, false));
        }
        ...
        private void ColumnDefinitionWithCharSet(string schema, string table, string name, ColumnOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            var srid = operation[SapHanaAnnotationNames.SpatialReferenceSystemId];
            if (srid is int &&
        ...
        private static string GetColumnTypeWithCharSetAndCollation(ColumnOperation operation, string columnType)
        {
            ...
            var charSet = operation[SapHanaAnnotationNames.CharSet];
            if (charSet != null)
            {
...
#pragma warning disable 618
            Debug.Assert(operation.FindAnnotation(SapHanaAnnotationNames.Collation) == null);
#pragma warning restore 618
            ...
            MigrationCommandListBuilder builder)
        {
            ...
            if (defaultValueSql is not null)
            {
                ...
                {
                    Dependencies.MigrationsLogger.DefaultValueNotSupportedWarning(defaultValueSql, columnType);
                }
            }
            ...
            [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            if (primaryKey != null)
            {
                ...
                // SapHana InnoDB has the requirement, that an AUTO_INCREMENT column has to be the first
                ...
                var sortedColumnNames = primaryKey.Columns.Length > 1
                    ? primaryKey.Columns
                        ...
                        .OrderBy(co => co[SapHanaAnnotationNames.ValueGenerationStrategy] is SapHanaValueGenerationStrategy generationStrategy
                                       && generationStrategy == SapHanaValueGenerationStrategy.IdentityColumn
                            ? 0
                            : 1)
                        .Select(co => co.Name)
                        .ToArray()
        ...
        protected override void Generate(
            ...
            bool terminate = true)
            => Generate(
                new SapHanaDropPrimaryKeyAndRecreateForeignKeysOperation
                {
                    IsDestructiveChange = operation.IsDestructiveChange,
                    Name = operation.Name,
                    Schema = operation.Schema,
        ...
        protected virtual void Generate(
            SapHanaDropPrimaryKeyAndRecreateForeignKeysOperation operation,
            IModel model,
            MigrationCommandListBuilder builder,
            bool terminate = true)
        {
            ...
            // if the foreign key columns are listed as the first columns and in the same order as in the foreign key (#678).
            // We therefore drop and later recreate all foreign keys to ensure, that no other dependencies on the
            // primary key exist, if explicitly requested by the user via `SapHanaMigrationBuilderExtensions.DropPrimaryKey()`.
            if (operation.RecreateForeignKeys)
            {
                TemporarilyDropForeignKeys(
                    model,
            ...
            MigrationCommandListBuilder builder)
        {
            ...
            var fullText = operation[SapHanaAnnotationNames.FullTextIndex] as bool?;
            if (fullText == true)
            {
                builder.Append("FULLTEXT ");
            }

            var spatial = operation[SapHanaAnnotationNames.SpatialIndex] as bool?;
            if (spatial == true)
            {
        ...
        protected override void IndexOptions(CreateIndexOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            // This is not supported by SapHana, so we don't call it here.

            var fullText = operation[SapHanaAnnotationNames.FullTextIndex] as bool?;
            if (fullText == true)
            {
                var fullTextParser = operation[SapHanaAnnotationNames.FullTextParser] as string;
                if (!string.IsNullOrEmpty(fullTextParser))
                {
                    // Official SapHana support exists since 5.1, but since MariaDB does not support full-text parsers and does not recognize
                    // the "/*!xxxxx" syntax for versions below 50700, we use 50700 here, even though the statement would work in lower
                    // versions as well. Since we don't support SapHana 5.6 officially anymore, this is fine.
        ...
        private string ColumnListWithIndexPrefixLength(MigrationOperation operation, string[] columns)
            => operation[SapHanaAnnotationNames.IndexPrefixLength] is int[] prefixValues
                ? ColumnList(
                    columns,
