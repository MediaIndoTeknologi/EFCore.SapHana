...
namespace WonderKidEngine.EntityFrameworkCore.SapHana.Migrations.Internal
{
    public class SapHanaMigrationsModelDiffer : MigrationsModelDiffer
    {
        // CHECK: Might be a good use case for runtime annotations.
        protected static class InternalLocalAnnotationNames
        {
            public const string InternalLocalPrefix = SapHanaAnnotationNames.Prefix + "Internal:MigrationsModelDiffer:";

            public const string ExecuteBefore = InternalLocalPrefix + "ExecuteBefore";
        }

        public SapHanaMigrationsModelDiffer(
        ...
        protected override IEnumerable<MigrationOperation> Add(IColumn target, DiffContext diffContext, bool inline = false)
        {
            if (!inline)
            {
                foreach (var propertyMapping in target.PropertyMappings)
                {
                    ...
                    {
                        var valueGenerationStrategy = SapHanaValueGenerationStrategyCompatibility.GetValueGenerationStrategy(
            ...
            IEnumerable<MigrationOperation> migrationOperations)
        {
            foreach (var operation in migrationOperations)
            {
                ...
                if (operation is not AlterColumnOperation alterColumnOperation ||
                    alterColumnOperation.OldColumn[SapHanaAnnotationNames.CharSet] is not string oldColumnCharSet ||
                    alterColumnOperation[SapHanaAnnotationNames.CharSet] is not string newColumnCharSet ||
            ...
            where TOperation : MigrationOperation
        {
            ...
            if (operation[SapHanaAnnotationNames.CollationDelegation] is DelegationModes databaseCollationDelegation)
            {
                // Don't leak the CollationDelegation annotation to the MigrationOperation.
                operation[SapHanaAnnotationNames.CollationDelegation] = null;
        ...
        private static void HandleCharSetDelegation(MigrationOperation operation, DelegationModes delegationModes)
        {
            ...
            if (operation[SapHanaAnnotationNames.CharSetDelegation] is DelegationModes charSetDelegation)
            {
                // Don't leak the CharSetDelegation annotation to the MigrationOperation.
                operation[SapHanaAnnotationNames.CharSetDelegation] = null;

                if (!charSetDelegation.HasFlag(delegationModes))
                {
                    operation[SapHanaAnnotationNames.CharSet] = null;
                }
            }
        }
