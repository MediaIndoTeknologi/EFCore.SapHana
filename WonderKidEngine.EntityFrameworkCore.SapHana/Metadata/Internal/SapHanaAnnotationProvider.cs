...
{
    public class SapHanaAnnotationProvider : RelationalAnnotationProvider
    {
        [NotNull] private readonly ISapHanaOptions _options;

        public SapHanaAnnotationProvider(
        ...
        public override IEnumerable<IAnnotation> For(IRelationalModel model, bool designTime)
        {
            ...
            // handled Collation property in SapHanaMigrationsModelDiffer.

            // Handle other annotations (including the delegation annotations).
            foreach (var annotation in model.Model.GetAnnotations()
                .Where(a => a.Name is SapHanaAnnotationNames.CharSetDelegation or
                                      SapHanaAnnotationNames.CollationDelegation))
            {
        ...
        public override IEnumerable<IAnnotation> For(ITable table, bool designTime)
        {
            ...
            foreach (var annotation in entityType.GetAnnotations()
                .Where(a => a.Name is SapHanaAnnotationNames.CharSetDelegation or
                                      SapHanaAnnotationNames.CollationDelegation or
                                      SapHanaAnnotationNames.StoreOptions))
            {
                yield return annotation;
            }
        }
        ...
        public override IEnumerable<IAnnotation> For(ITableIndex index, bool designTime)
        {
            ...
            {
                yield return new Annotation(
                    SapHanaAnnotationNames.FullTextIndex,
                    isFullText.Value);
            }
            ...
            {
                yield return new Annotation(
                    SapHanaAnnotationNames.FullTextParser,
                    fullTextParser);
            }
            ...
            {
                yield return new Annotation(
                    SapHanaAnnotationNames.SpatialIndex,
                    isSpatial.Value);
            }
        }
        ...
        public override IEnumerable<IAnnotation> For(IColumn column, bool designTime)
        {
            ...
            if (column.PropertyMappings.Where(
                ...
                .Select(m => m.Property)
                .FirstOrDefault(p => p.GetValueGenerationStrategy(table) == SapHanaValueGenerationStrategy.IdentityColumn) is IProperty identityProperty)
            {
                var valueGenerationStrategy = identityProperty.GetValueGenerationStrategy(table);
                yield return new Annotation(
                    SapHanaAnnotationNames.ValueGenerationStrategy,
                    valueGenerationStrategy);
            }
            else if (properties.FirstOrDefault(
                p => p.GetValueGenerationStrategy(table) == SapHanaValueGenerationStrategy.ComputedColumn) is IProperty computedProperty)
            {
            ...
            {
                yield return new Annotation(
                    SapHanaAnnotationNames.SpatialReferenceSystemId,
                    srid);
            }
        }
        ...
        protected virtual string GetActualPropertyCharSet(IProperty[] properties, DelegationModes currentLevel)
        {
            return properties.Select(p => p.GetCharSet()).FirstOrDefault(s => s is not null) ??
                   properties.Select(
                           p => p.FindTypeMapping() is SapHanaStringTypeMapping {IsNationalChar: false}
        ...
        protected virtual string GetActualPropertyCollation(IProperty[] properties, DelegationModes currentLevel)
        {
            ...
            // We have been using the `SapHana:Collation` annotation before EF Core added collation support.
            // Our `SapHanaPropertyExtensions.GetSapHanaLegacyCollation()` method handles the legacy case, so we explicitly
            ...
            return properties.All(p => p.GetCollation() is null)
                ? properties.Select(p => p.GetSapHanaLegacyCollation()).FirstOrDefault(c => c is not null) ??
                  properties.Select(
