...
{
    public class SapHanaSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
    {
        ...
        protected static readonly MethodInfo[] NewArrayExpressionSupportMethodInfos = Array.Empty<MethodInfo>()
            .Concat(typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethods().Where(m => m.Name == nameof(SapHanaDbFunctionsExtensions.Match)))
        ...
        public SapHanaSqlTranslatingExpressionVisitor(
        ...
        protected override Expression VisitUnary(UnaryExpression unaryExpression)
        {
            if (unaryExpression.NodeType == ExpressionType.ArrayLength)
            {
                ...
                if (sqlOperand.Type == typeof(byte[]) &&
                    (sqlOperand.TypeMapping is null or SapHanaByteArrayTypeMapping))
                {
                ...
                unaryExpression.NodeType == ExpressionType.ConvertChecked)
            {
                ...
                if (visitedOperand is SapHanaJsonTraversalExpression traversal)
                {
            ...
            {
                // SapHana implicitly casts numbers used in BITWISE NOT operations (~ operator) to BIGINT UNSIGNED.
        ...
        {
            // Needed for SapHanaDbFunctionsExtensions.Match() and String.Concat() translation.
            ...
            {
                var typeMapping = ((SapHanaStringTypeMapping)Dependencies.TypeMappingSource.GetMapping(typeof(string))).Clone(forceToString: true);
                return _sqlExpressionFactory.ComplexFunctionArgument(
        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            if (result == QueryCompilationContext.NotTranslatedExpression &&
                SapHanaStringComparisonMethodTranslator.StringComparisonMethodInfos.Any(m => m == methodCallExpression.Method))
            {
                var message = SapHanaStrings.QueryUnableToTranslateMethodWithStringComparison(
