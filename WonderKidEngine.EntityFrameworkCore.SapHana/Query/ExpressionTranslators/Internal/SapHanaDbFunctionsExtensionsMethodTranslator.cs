...
namespace WonderKidEngine.EntityFrameworkCore.SapHana.Query.ExpressionTranslators.Internal
{
    ...
    /// </summary>
    public class SapHanaDbFunctionsExtensionsMethodTranslator : IMethodCallTranslator
    {
        ...
        private static readonly MethodInfo[] _likeMethodInfos
            = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethods()
                .Where(method => method.Name == nameof(SapHanaDbFunctionsExtensions.Like)
        ...
        private static readonly MethodInfo _matchMethodInfo
            = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(
                nameof(SapHanaDbFunctionsExtensions.Match),
                new[] {typeof(DbFunctions), typeof(string), typeof(string), typeof(SapHanaMatchSearchMode)});
        ...
        private static readonly MethodInfo[] _hexMethodInfos
            = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethods()
                .Where(method => method.Name == nameof(SapHanaDbFunctionsExtensions.Hex) &&
                                 method.IsGenericMethod)
                .SelectMany(method => _supportedHexTypes.Select(type => method.MakeGenericMethod(type)))
                .ToArray();

        private static readonly MethodInfo _unhexMethodInfo = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(nameof(SapHanaDbFunctionsExtensions.Unhex), new[] {typeof(DbFunctions), typeof(string)});

        private static readonly MethodInfo _degreesDoubleMethodInfo = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(nameof(SapHanaDbFunctionsExtensions.Degrees), new[] { typeof(DbFunctions), typeof(double) });
        private static readonly MethodInfo _degreesFloatMethodInfo = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(nameof(SapHanaDbFunctionsExtensions.Degrees), new[] { typeof(DbFunctions), typeof(float) });

        private static readonly MethodInfo _radiansDoubleMethodInfo = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(nameof(SapHanaDbFunctionsExtensions.Radians), new[] { typeof(DbFunctions), typeof(double) });
        private static readonly MethodInfo _radiansFloatMethodInfo = typeof(SapHanaDbFunctionsExtensions).GetRuntimeMethod(nameof(SapHanaDbFunctionsExtensions.Radians), new[] { typeof(DbFunctions), typeof(float) });

        public SapHanaDbFunctionsExtensionsMethodTranslator(ISqlExpressionFactory sqlExpressionFactory)
        {
                ...
                Equals(method, _matchWithMultiplePropertiesMethodInfo))
            {
                if (arguments[3] is SqlConstantExpression constant)
                {
                    return _sqlExpressionFactory.MakeMatch(
                        arguments[1],
                        arguments[2],
                        (SapHanaMatchSearchMode)constant.Value);
                }
                ...
                if (arguments[3] is SqlParameterExpression parameter)
                {
                    ...
                    var andClauses = Enum.GetValues(typeof(SapHanaMatchSearchMode))
                        .Cast<SapHanaMatchSearchMode>()
