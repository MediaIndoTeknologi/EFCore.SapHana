...
{
    public class SapHanaStringMethodTranslator : IMethodCallTranslator
    {
        ...
        public SapHanaStringMethodTranslator(
            SapHanaSqlExpressionFactory sqlExpressionFactory,
            SapHanaTypeMappingSource typeMappingSource,
            ...
            IDiagnosticsLogger<DbLoggerCategory.Query> logger)
        {
            ...
            {
                return new SapHanaStringComparisonMethodTranslator(_sqlExpressionFactory, _options)
            ...
            if (_replaceMethodInfo.Equals(method))
            {
                ...
                // Due to a bug in all versions of MariaDB and all SapHana versions below 8.0.x (exact version that fixed the issue is
                    ...
                    : null) ?? method))
            {
                ...
                // is translated.
                var concatArguments = arguments[0] is SapHanaComplexFunctionArgumentExpression sapHanaComplexFunctionArgumentExpression
                    ? new SqlExpression[] { sapHanaComplexFunctionArgumentExpression }
                    : arguments.Select(
                            e => e switch
                            {
                                SqlConstantExpression c => _sqlExpressionFactory.Constant(c.Value.ToString()),
                                SqlParameterExpression p => p.ApplyTypeMapping(
                                    ((SapHanaStringTypeMapping)_typeMappingSource.GetMapping(typeof(string))).Clone(forceToString: true)),
                                _ => e,
                            })
                        .ToArray();

                // We haven't implemented expansion of SapHanaComplexFunctionArgumentExpression yet, so the default nullability check would
                ...
                return _sqlExpressionFactory.NullableFunction(
                    ...
                    concatArguments,
                    method.ReturnType,
                    onlyNullWhenAnyNullPropagatingArgumentIsNull: arguments[0] is not SapHanaComplexFunctionArgumentExpression);
            }
