...
{
    // TODO: Revamp
    public class SapHanaUpdateSqlGenerator : UpdateSqlGenerator, ISapHanaUpdateSqlGenerator
    {
        /// <summary>
        ...
        /// </summary>
        public SapHanaUpdateSqlGenerator(
            ...
            int commandPosition)
        {
            ...
            if (modificationCommands.Count == 1
                && modificationCommands[0].ColumnModifications.All(o =>
                    !o.IsKey
                    || !o.IsRead
                    || o.Property?.GetValueGenerationStrategy(table) == SapHanaValueGenerationStrategy.IdentityColumn))
            {
            ...
            var nonIdentityOperations = modificationCommands[0].ColumnModifications
                .Where(o => o.Property?.GetValueGenerationStrategy(table) != SapHanaValueGenerationStrategy.IdentityColumn)
                .ToList();
            ...
            IReadOnlyList<IColumnModification> operations)
        {
            ...
            {
                // An empty column and value list signales SapHana that only default values should be used.
            ...
            IReadOnlyList<IColumnModification> operations)
        {
            ...
            // Because we currently use `SapHanaValueGenerationStrategy.IdentityColumn` for auto_increment columns as well
            // as CURRENT_TIMESTAMP columns, we need to use `SapHanaPropertyExtensions.IsCompatibleAutoIncrementColumn()`
            // to ensure, that the column is actually an auto_increment column.
            // See https://github.com/PomeloFoundation/WonderKidEngine.EntityFrameworkCore.SapHana/issues/1300
            var nonDefaultOperations = operations
                .Where(
                    o => !o.IsKey ||
                         ...
                         SapHanaPropertyExtensions.IsCompatibleAutoIncrementColumn(o.Property))
                .ToList()
                .AsReadOnly();
